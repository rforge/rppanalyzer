...
An example sample description file describing a serially diluted sample set
is included.
...
The second element is analogously built up but with foreground expression data corrected to dilution intercepts via the \Rfunction{correctDilinterc} function. 
...
## import raw data
fgRaw.tmp <- read.delim("test_dataexpression.txt",stringsAsFactors=
                          FALSE,row.names=NULL,header=TRUE)
...
correctedData<-correctDilinterc(dilseries=fgRaw[which(
  fgRaw$sample_type=="control" & !is.na(fgRaw$dilSeriesID)),],
                                arraydesc=rawdata$arraydescription,
                                timeseries=fgRaw[which(
                                  fgRaw$sample_type=="measurement"),],
                                exportNo=2)
...
res <- dataPreproc(dataDir=getwd(),blocks=12,spot="aushon",exportNo=2)
## remove arrays
