...
Another recommendable way is to apply the \Rfunction{correctDilinterc} function either directly or within the data preprocessing function \Rfunction{dataPreproc}. It derives intercepts of dilution series in dependence of \Rfunarg{dilSeriesID}, defined in the \emph{sampledescription} file, as well as \Rfunarg{slide}, \Rfunarg{pad}, \Rfunarg{incubation\_run} and \Rfunarg{spotting\_run}, defined in the data frame holding the array information. A smoothing spline is used to extrapolate to zero. Nonparametric bootstrap is used to estimate uncertainty of the intercept estimate. Linear models are applied to the intercepts as response variables in dependence of diverse predictors, namely simply a constant, the antibody, antibody + slide or antibody + slide + sample. Via Analysis of Variances (ANOVA) it is tested which model fits best. Finally the dilution intercept estimation is subtracted from the original foreground expression.
...
?calcSdc
@
For the usage of the \Rfunction{correctDilinterc} function we recommend the data preprocessing function \Rfunction{dataPreproc}.
...
Among all possible correction and normalization methods prior to actual data analysis, we recommend the correction based on serially diluted samples via \Rfunction{correctDilinterc}, combined with normalization according to total protein concentration via FCF. These important preprocessing steps are implemented in the \Rfunction{dataPreproc} function. It imports the raw data, corrects and normalizes it and generates plots for quality checks.
@
